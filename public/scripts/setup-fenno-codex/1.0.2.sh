...
if ( : <"$confirmation_input" ) 2>/dev/null; then
  ...
  printf '%s\n' '当前环境无法交互确认，已保留当前 Codex 进程。'
fi

case "$close_codex" in
  y|Y)
    if ! command -v pkill >/dev/null 2>&1; then
      printf '%s\n' '未找到 pkill 命令，无法关闭 Codex 进程。'
    else
      codex_process_found=0
      pkill -x codex 2>/dev/null && codex_process_found=1 || :
      pkill -x Codex 2>/dev/null && codex_process_found=1 || :
      if [ "$(uname -s 2>/dev/null || :)" = Darwin ]; then
        pkill -f '/Codex.app/Contents/' 2>/dev/null && codex_process_found=1 || :
      fi
      if [ "$codex_process_found" -eq 1 ]; then
        printf '%s\n' '已关闭所有 Codex 进程。'
      else
        printf '%s\n' '未发现正在运行的 Codex 进程。'
      fi
    ...
    ;;
  *)
    if [ "$confirmation_available" -eq 1 ]; then
      printf '%s\n' '已保留当前 Codex 进程。'
    fi
    ;;
esac
