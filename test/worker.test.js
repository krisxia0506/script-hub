...
const assets = new Map([
  ...
  ['/scripts/setup-fenno-codex.sh', '#!/bin/sh\necho fenno\n'],
...
test('GET Fenno Codex alias returns shell content type', async () => {
  const response = await worker.fetch(
    new Request('https://example.test/setup-fenno-codex'),
    makeEnv(),
  );
...
test('GET removed Fenno models alias returns 404', async () => {
  const response = await worker.fetch(
